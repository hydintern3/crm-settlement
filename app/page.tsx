...
import { EMPTY_SNAPSHOT, normalizeSnapshot, summarizeRows, type BusinessRow, type NumericValue, type Snapshot } from "./lib/data-model";
...
export default function Home() {
  ...
  if (activeNav === "总览") content = <><Metrics rows={filteredRows} /><section className="dashboard-grid"><Panel label="MONTHLY TREND" title="月平均计量趋势"><MonthlyChart data={snapshot.monthly} /></Panel><Panel label="METERING RULE" title="计量规则分布" aside={<span className="tag">点击可下钻</span>}><RuleChart data={snapshot.meteringRules} onSelect={selectRule} /></Panel><Panel label="SALES PERFORMANCE" title="负责人业绩"><RankingChart items={snapshot.owners} onSelect={(name) => selectRank(name, "owner")} /></Panel><Panel label="PROVIDER RANKING" title="服务商进单"><RankingChart items={snapshot.providers} onSelect={(name) => selectRank(name, "provider")} /></Panel></section></>;
  ...
  else if (activeNav === "业务分析") content = <><Metrics rows={filteredRows} /><section className="dashboard-grid"><Panel label="BUSINESS TREND" title="月度计量趋势"><MonthlyChart data={snapshot.monthly} /></Panel><Panel label="RULE STRUCTURE" title="计量规则结构"><RuleChart data={snapshot.meteringRules} onSelect={selectRule} /></Panel></section></>;
  else if (activeNav === "销售分析") content = <section className="module-grid"><Panel label="OWNER RANKING" title="负责人业绩排名"><RankingChart items={snapshot.owners} onSelect={(name) => selectRank(name, "owner")} /></Panel><Panel label="OWNER DETAILS" title="负责人业务明细"><DataTable rows={filteredRows} pageSize={10} /></Panel></section>;
  else if (activeNav === "服务商分析") content = <section className="module-grid"><Panel label="PROVIDER RANKING" title="服务商进单排名"><RankingChart items={snapshot.providers} onSelect={(name) => selectRank(name, "provider")} /></Panel><Panel label="PROVIDER DETAILS" title="服务商业务明细"><DataTable rows={filteredRows} pageSize={10} /></Panel></section>;
