...
export function BusinessProgressTables({ rows, rules }: { rows: BusinessRow[]; rules: CalculationRuleConfig }) {
  ...
  return <section className="module-grid report-module-grid"><ReportPanel wide label="BUSINESS PROGRESS" title="业务进展多维分析" status="partial" description={`新增仅统计当前计量规则为“新增量”的记录；同一套指标支持公司、销售、供应商、I服务商和II服务商切换；当前拆机率口径：${denominatorText}。净增长月平均计量＝新增月平均计量－拆机月平均计量；净增月平均资费＝净增长月平均计量÷净增长线路数，净增长线路数为0时显示--。`}>
...
export function DoubleLineOverview({ rows, year = "2026" }: { rows: BusinessRow[]; year?: string }) {
  ...
  const [period, setPeriod] = useState<"year" | "quarter" | "custom">("quarter");
  ...
  const periodBounds = period === "year" ? { start: `${year}-01-01`, end: `${year}-12-31` }
    : period === "quarter" ? { start: `${year}-${String((quarterNumber - 1) * 3 + 1).padStart(2, "0")}-01`, end: `${year}-${String(quarterNumber * 3).padStart(2, "0")}-31` }
      : customStart && customEnd ? { start: customStart, end: customEnd } : undefined;
  const periodAssessment = buildDoubleLineAssessment(rows, actualTarget, periodBounds);
  const formatRatio = (value: NumericValue) => value === null ? "--" : `${number(value, 1)}%`;
  const rowsToDisplay = [{
    key: "assessment",
    installLines: number(periodAssessment.installLines), installConvertibleRecords: number(periodAssessment.installConvertibleRecords), installConvertedLines: number(periodAssessment.installConvertedLines), installTotalLines: number(periodAssessment.installTotalLines),
    removalLines: number(periodAssessment.removalLines), removalConvertibleRecords: number(periodAssessment.removalConvertibleRecords), removalConvertedLines: number(periodAssessment.removalConvertedLines), removalTotalLines: number(periodAssessment.removalTotalLines),
    rawRatio: formatRatio(periodAssessment.rawRatio), convertedRatio: formatRatio(periodAssessment.convertedRatio), rawPendingLines: number(periodAssessment.rawPendingLines), convertedPendingLines: number(periodAssessment.convertedPendingLines),
  }];
  return <ReportPanel wide label="DOUBLE-LINE ASSESSMENT" title="双线拆装比" description={`新增仅取当前计量规则为“新增量”的记录，日期按初始完工日期；拆机按业务属性，日期按完工日期。当前${period === "year" ? `${year}年度` : period === "quarter" ? `${year}年${quarter}` : `${customStart}至${customEnd}`}考核。折算仅取 CRM 源表“月平均计量”：低于10,000元不折算；10,000至11,999元计1线，此后每满2,000元增加1线，单条最高20线。`}>
    <div className="dimension-tabs" role="group" aria-label="拆装比时间范围"><span className="section-label">时间范围</span><select value={period} onChange={(event) => setPeriod(event.target.value as "year" | "quarter" | "custom")}><option value="year">年度</option><option value="quarter">季度</option><option value="custom">自定义区间</option></select>{period === "quarter" && <select value={quarter} onChange={(event) => setQuarter(event.target.value)}><option value="Q1">第一季度</option><option value="Q2">第二季度</option><option value="Q3">第三季度</option><option value="Q4">第四季度</option></select>}{period === "custom" && <><label><span className="sr-only">开始日期</span><input type="date" value={customStart} onChange={(event) => setCustomStart(event.target.value)} /></label><span>至</span><label><span className="sr-only">结束日期</span><input type="date" value={customEnd} onChange={(event) => setCustomEnd(event.target.value)} /></label></>}</div>
...
export function SalesReportTables({ rows, year = "2026" }: { rows: BusinessRow[]; year?: string }) {
  ...
  return <section className="module-grid report-module-grid">
    ...
    <ReportPanel wide label="NET GROWTH" title={`${year} 年度销售业务净增情况`} description={`新增口径：当前计量规则为“新增量”。净增扣减“当年新增当年拆机”：同时符合新增口径、业务属性为拆机且 CRM 完工日期在 ${year} 年。末列“${year} 年度拆机”仅按业务属性为拆机且 CRM 完工日期在 ${year} 年统计，不混入净增。`}>
      <ReportTable columns={[{ key: "businessCategory", label: "业务种类" }, { key: "owner", label: "负责人" }, { key: "additions", label: `${year} 年度新增线数`, numeric: true }, { key: "additionAmount", label: "新增业务量", numeric: true }, { key: "sameYearRemovals", label: "当年新增当年拆机线数", numeric: true }, { key: "sameYearRemovalAmount", label: "当年新增当年拆机业务量", numeric: true }, { key: "netLines", label: `${year} 年度净增线数`, numeric: true }, { key: "netAmount", label: "净增业务量", numeric: true }, { key: "annualRemovals", label: `${year} 年度拆机线数`, numeric: true }, { key: "annualRemovalAmount", label: "年度拆机业务量", numeric: true }, { key: "ratio", label: "当年新增拆机率", numeric: true }]} rows={netGrowth} summary={netSummary} />
    </ReportPanel>
  </section>;
}
...
export function AnnualAdditionReconciliation({ rows, year = "2026" }: { rows: BusinessRow[]; year?: string }) {
  ...
  const tableRows = data.map((item) => ({
    key: item.key, label: item.label, records: number(item.records), lines: number(item.lines), amount: money(item.monthlyMetering), tariff: money(item.monthlyTariff), active: number(item.activeLines), activeAmount: money(item.activeMonthlyMetering), activeTariff: money(item.activeMonthlyTariff), nonActive: number(item.nonActiveLines), nonActiveAmount: money(item.nonActiveMonthlyMetering), nonActiveTariff: money(item.nonActiveMonthlyTariff), sameYearRemoval: number(item.sameYearRemovalLines), otherRemoval: number(item.laterRemovalLines), removalDateMissing: number(item.removalDateMissingLines), inactive: number(item.inactiveNotRemovalLines), statusMissing: number(item.statusMissingLines), reconciled: number(item.reconciledLines), unreconciled: number(item.unreconciledLines),
  }));
  const totalLines = data.reduce((sum, item) => sum + item.lines, 0);
  const summary = data.length ? {
    key: "summary", label: "总计 / 总览", records: number(data.reduce((sum, item) => sum + item.records, 0)), lines: number(totalLines), amount: money(knownSum(data.map((item) => item.monthlyMetering))), tariff: money(knownSum(data.map((item) => item.monthlyTariff))), active: number(data.reduce((sum, item) => sum + item.activeLines, 0)), activeAmount: money(knownSum(data.map((item) => item.activeMonthlyMetering))), activeTariff: money(knownSum(data.map((item) => item.activeMonthlyTariff))), nonActive: number(data.reduce((sum, item) => sum + item.nonActiveLines, 0)), nonActiveAmount: money(knownSum(data.map((item) => item.nonActiveMonthlyMetering))), nonActiveTariff: money(knownSum(data.map((item) => item.nonActiveMonthlyTariff))), sameYearRemoval: number(data.reduce((sum, item) => sum + item.sameYearRemovalLines, 0)), otherRemoval: number(data.reduce((sum, item) => sum + item.laterRemovalLines, 0)), removalDateMissing: number(data.reduce((sum, item) => sum + item.removalDateMissingLines, 0)), inactive: number(data.reduce((sum, item) => sum + item.inactiveNotRemovalLines, 0)), statusMissing: number(data.reduce((sum, item) => sum + item.statusMissingLines, 0)), reconciled: number(data.reduce((sum, item) => sum + item.reconciledLines, 0)), unreconciled: number(data.reduce((sum, item) => sum + item.unreconciledLines, 0)),
  } : undefined;
  return <section className="module-grid report-module-grid"><ReportPanel wide label="ANNUAL ADDITION RECONCILIATION" title={`${year} 年度新增复核`} status="partial" description={`新增原始线路数仅按初始完工日期在 ${year} 年统计，不受全局年份、业务属性、计量规则或活跃状态筛选影响。年度新增未留存/已拆机差额＝年度新增总量－当前活跃，可能包含已拆机及其他当前非活跃或待复核记录。`}>
    ...
    <ReportTable columns={[{ key: "label", label: "复核维度" }, { key: "records", label: "新增业务数", numeric: true }, { key: "lines", label: "年度新增总量", numeric: true }, { key: "amount", label: "新增月平均计量", numeric: true }, { key: "tariff", label: "新增月平均资费", numeric: true }, { key: "active", label: "当前活跃线路数", numeric: true }, { key: "activeAmount", label: "当前活跃月平均计量", numeric: true }, { key: "activeTariff", label: "当前活跃月平均资费", numeric: true }, { key: "nonActive", label: "年度新增未留存/已拆机差额", numeric: true }, { key: "nonActiveAmount", label: "差额月平均计量", numeric: true }, { key: "nonActiveTariff", label: "差额月平均资费", numeric: true }, { key: "sameYearRemoval", label: "当年新增当年拆机", numeric: true }, { key: "otherRemoval", label: "其他年度拆机", numeric: true }, { key: "removalDateMissing", label: "拆机日期缺失", numeric: true }, { key: "inactive", label: "当前非活跃未拆机", numeric: true }, { key: "statusMissing", label: "活跃状态缺失", numeric: true }, { key: "reconciled", label: "已分层合计", numeric: true }, { key: "unreconciled", label: "待复核差额", numeric: true }]} rows={tableRows} summary={summary} emptyText={`当前版本没有初始完工日期在 ${year} 年的记录`} />
  </ReportPanel></section>;
}
