...
export function ConfigurableChart({ data, template, onSelect }: { data: AggregatedChartData; template: ChartTemplateDraft; onSelect?: (name: string) => void }) {
  ...
  if (template.chartType === "pie" || template.chartType === "donut") {
    ...
    const showDualAxis = !template.seriesField && (template.chartType === "bar" || template.chartType === "combo") && template.measures.length === 2 && measureUnits[0] !== measureUnits[1];
    ...
    option = {
      ...
      dataZoom: zoom,
      series: data.series.map((series, index) => isLine ? ({ name: series.name, type: "line", xAxisIndex: horizontal && showDualAxis ? index : 0, yAxisIndex: horizontal ? 0 : showDualAxis ? index : 0, smooth: template.options.smooth, connectNulls: false, symbolSize: 7, areaStyle: template.chartType === "area" ? { opacity: .14 } : undefined, label: { show: template.options.showLabels, formatter: (params: unknown) => formatDataValue((params as { value?: unknown }).value) }, labelLayout: { hideOverlap: false }, data: series.values }) : ({ name: series.name, type: "bar", xAxisIndex: horizontal && showDualAxis ? index : 0, yAxisIndex: horizontal ? 0 : showDualAxis ? index : 0, stack: template.chartType === "stackedBar" ? "total" : undefined, barMaxWidth: showDualAxis ? 15 : 30, barGap: showDualAxis ? "35%" : undefined, barCategoryGap: showDualAxis ? "30%" : undefined, label: { show: template.options.showLabels, position: horizontal ? "right" : "top", formatter: (params: unknown) => formatDataValue((params as { value?: unknown }).value) }, labelLayout: { hideOverlap: false }, itemStyle: { borderRadius: horizontal ? [0, 4, 4, 0] : [4, 4, 0, 0] }, data: series.values })),
    };
  }
