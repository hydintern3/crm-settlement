...
export function MonthlyChart({ data }: { data: Snapshot["monthly"] }) {
  ...
  const option: EChartsCoreOption = {
    ...
    tooltip: { trigger: "axis", textStyle: { fontFamily: CHART_FONT, fontSize: 12 }, valueFormatter: (value: unknown) => value === null ? "--" : `¥ ${Number(value).toLocaleString("zh-CN")}` },
...
export function RankingChart({ items, onSelect }: { items: RankedItem[]; onSelect?: (name: string) => void }) {
  ...
  const option: EChartsCoreOption = {
    ...
    tooltip: { trigger: "axis", axisPointer: { type: "shadow" }, textStyle: { fontFamily: CHART_FONT, fontSize: 12 }, valueFormatter: (value: unknown) => `¥ ${Number(value).toLocaleString("zh-CN")}` },
