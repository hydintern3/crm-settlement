...
async function readDashboard(): Promise<DashboardFile> {
  ...
  try {
    ...
    const templates = parsed.templates.map((template) => {
      ...
      const corrected = template.id === "system-provider-ranking" && sourceSchemaVersion < 9
        ? { ...migrated, title: "供应商分布", description: "按供应商汇总线路数与月平均计量，双横轴分组柱状展示", chartType: "bar" as const, dimension: { field: "provider" as const }, measures: [{ field: "lines" as const, aggregation: "sum" as const }, { field: "monthlyMetering" as const, aggregation: "sum" as const }], options: { ...migrated.options, orientation: "horizontal" as const, smooth: false } }
        : template.id === "system-service-ranking" && sourceSchemaVersion < 9
          ? { ...migrated, title: "服务商分布", description: "按 I 服务编号汇总线路数与月平均计量，双横轴分组柱状展示", chartType: "bar" as const, dimension: { field: "serviceCode" as const }, measures: [{ field: "lines" as const, aggregation: "sum" as const }, { field: "monthlyMetering" as const, aggregation: "sum" as const }], options: { ...migrated.options, topN: 12, orientation: "horizontal" as const, showLabels: false, showOther: false, size: "wide" as const, height: 420, smooth: false } }
          : migrated;
      return {
        ...template,
        ...corrected,
