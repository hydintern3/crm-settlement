...
export const CHART_SCHEMA_VERSION = 9;
...
export const DEFAULT_CHART_TEMPLATES: readonly ChartTemplate[] = [
  ...
  systemTemplate("system-provider-ranking", { ...defaultChartDraft(40), title: "供应商分布", description: "按供应商汇总线路数与月平均计量，双横轴分组柱状展示", chartType: "bar", dimension: { field: "provider" }, measures: [{ field: "lines", aggregation: "sum" }, { field: "monthlyMetering", aggregation: "sum" }], options: { ...defaultChartDraft().options, orientation: "horizontal" } }),
  systemTemplate("system-service-ranking", { ...defaultChartDraft(50), title: "服务商分布", description: "按 I 服务编号汇总线路数与月平均计量，双横轴分组柱状展示", chartType: "bar", dimension: { field: "serviceCode" }, measures: [{ field: "lines", aggregation: "sum" }, { field: "monthlyMetering", aggregation: "sum" }], options: { ...defaultChartDraft().options, orientation: "horizontal", size: "wide", height: 420, showLabels: false, smooth: false } }),
] as const;
