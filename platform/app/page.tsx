...
function exportRows(rows: BusinessRow[]) {
  const headers = ["设备编号", "业务属性判断（平台）", "判断依据（平台）", "业务属性（CRM）", "业务名称", "负责人", "供应商", "I服务编号", "I服务简称", "II服务编号", "II服务简称", "初始完工日期", "完工日期", "统计完工日期（平台）", "日期取值来源（平台）", "表内现日期", "活跃状态", "计量规则（按当前日期计算）", "计量规则来源（平台）", "计算状态", "分期计算标识", "拆机类型", "用户拆机原因", "联系人固话（脱敏）", "付款周期", "服务分类", "线数", "月平均计量（原值）", "月平均计量（万元）", "优惠资费", "营销增值费用", "是否低于授权价", "业务毛利"];
...
function DataTable({ rows, pageSize = 20 }: { rows: BusinessRow[]; pageSize?: number }) {
  ...
  return <>
    <div className="table-scroll"><table><thead><tr><th>设备编号</th><th>业务属性判断（平台）</th><th>判断依据（平台）</th><th>业务属性（CRM）</th><th>业务名称</th><th>负责人</th><th>供应商</th><th>I服务编号</th><th>I服务简称</th><th>II服务编号</th><th>II服务简称</th><th>统计完工日期（平台）</th><th>日期取值来源（平台）</th><th>活跃状态</th><th>计量规则（按当前日期计算）</th><th>计量规则来源（平台）</th><th>计算状态</th><th>分期计算标识</th><th>拆机类型</th><th>用户拆机原因</th><th>联系人固话（脱敏）</th><th>付款周期</th><th>服务分类</th><th className="number">线数</th><th className="number">月平均计量（万元）</th></tr></thead>
