...
- 管理员密码只保存为 scrypt 摘要，会话使用 HttpOnly、SameSite=Strict 的签名 Cookie；生产必须使用 HTTPS。账号、密码摘要和会话密钥只能通过运行环境注入。
- 上传原文件、快照、版本清单和审计日志只允许保存在 `/app/data` 持久卷，不提供原文件下载接口，不得写入静态目录、镜像、日志或 Git。
...
- GHCR 镜像只允许由 `main` 分支 CI 在完整验证和容器健康检查通过后发布；云主机应使用 `latest` 更新，并保留提交 SHA 标签用于可复核回滚。
