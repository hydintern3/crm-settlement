...
内部数据工作台，支持管理员登录、服务器数据版本、组合筛选、双线业务指标、负责人/供应商/服务商排名、明细查询和可扩展报表注册。
...
- 多版本按设备编号整合时，较新版本的非空字段覆盖已有值，较新空值不会擦除完整版本中的字段；修复前生成的整合版本需从原始版本重新生成。
...
- 默认提供月平均计量趋势、计量规则分布、负责人业绩、供应商进单和按 I 服务编号汇总的服务商进单五个系统模板；图表数值标签始终显示。
