...
- 分支：`main`
- 当前提交：`0cc8e4a`
- 提交说明：`feat: 更新图表模板和迁移逻辑，支持从 Top 10 迁移到 Top 50，优化模板解析和验证`
- 当前未提交改动：2026-08-19 六项修复，包含字段级版本整合、万元六位小数、供应商/服务商拆分、图表常显数字、核心字段导出和分析表总计；尚未提交。
...
3. 改进 `.github/workflows/ci.yml`，让离线镜像包同时包含 `latest` 与提交 SHA 标签，避免每次在服务器手工补标签。此项尚未实施。
