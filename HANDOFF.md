...
- 分支：`main`
- 当前提交：`f65468a`
- 提交说明：`feat: add supplier support and enhance data handling`
- 当前未提交改动：新增离线镜像一键部署脚本及相应文档、守护测试和长期部署规则；尚未提交。
...
在提交 `f65468a` 加当前未提交的一键离线部署改动上已完成：

- `npm run verify`：通过。
- `bash -n deploy/offline-deploy.sh`：通过；真实 Docker/云主机部署尚未执行。
- 自动化测试：16 项通过；覆盖稀疏新版本不擦除既有字段、万元图表聚合、旧图表常显标签迁移、版本持久化和 standalone 服务。
- 依赖安全检查：无 high 级别漏洞。
...
1. 提交并构建包含本轮六项修复的新镜像；部署后必须从原始活跃/不活跃版本重新生成整合版本，旧整合版本不会自动修复。
